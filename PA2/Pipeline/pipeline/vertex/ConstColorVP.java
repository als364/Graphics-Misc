...
public class ConstColorVP extends VertexProcessor {
     ...
     *      Vector2f[], Vertex[])
     */
    public void triangle(Vector3f[] vs, Color3f[] cs, Vector3f[] ns_ign,
            Vector2f[] ts_ign, Vertex[] output) {
        for (int k = 0; k < 3; k++) {
            vertex(vs[k], cs[k], null, null, output[k]);
        }
    }

    public void vertex(Vector3f v, Color3f c, Vector3f n_ign, Vector2f t_ign,
            Vertex output) {
        // Set the vertex by multiplying by the Model View Projection Matrix
        // output.v.set(v.x, v.y, v.z, 1);
        // m.rightMultiply(output.v);
        Util.ftransform(output, v, m);
        output.c.set(c);
    }
}
